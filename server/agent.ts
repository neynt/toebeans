...
export async function runAgentTurn(
  ...
  if (persistToSession) {
    ...
    await options.onUserMessagePersisted?.()
  }

  const toolContext: ToolContext = { sessionId, workingDir, outputTarget, route: options.route, abortSignal: options.abortSignal }
  ...
  while (true) {
    ...
    if (options.checkAbort?.()) {
      console.log(`[agent] turn ended: abort_before_llm (session=${sessionId})`)
    ...
    let hasText = false
    let rawResponseOutput: unknown[] | undefined
    let streamEndReason: string | null = null
    let streamEndDetails: Record<string, unknown> | undefined
    ...
    try {
      for await (const chunk of provider.stream({ messages, system, tools: toolDefs, abortSignal: options.abortSignal })) {
        ...
        switch (chunk.type) {
          ...
          case 'raw_output':
            rawResponseOutput = chunk.items
            break

          case 'stream_end':
            streamEndReason = chunk.reason
            streamEndDetails = chunk.details
            break
        }
      }
    } catch (err) {
    ...
    if (streamAborted) {
      console.log(`[agent] turn ended: stream_aborted (session=${sessionId}, stream=${streamEndReason ?? 'n/a'})`)
    ...
    if (assistantContent.length > 0) {
      const assistantMessage: Message = { role: 'assistant', content: assistantContent }
      // only persist rawResponseOutput when it actually has items. an empty
      // array slips past the buildInput check in the codex provider and causes
      // the assistant's tool_uses to be silently dropped from the next request,
      // putting the agent in a doom loop where it re-runs the same tool calls
      // forever because it can't see its own history.
      if (rawResponseOutput && (rawResponseOutput as unknown[]).length > 0) {
        assistantMessage.rawResponseOutput = rawResponseOutput
      }
      messages.push(assistantMessage)
      if (persistToSession) {
        await appendMessage(sessionId, assistantMessage, cost)
    ...
    // log iteration summary for debugging agent loop flow
    const numTextBlocks = assistantContent.filter(b => b.type === 'text').length
    const numToolUseBlocks = assistantContent.filter(b => b.type === 'tool_use').length
    console.log(`[agent] iteration: ${numTextBlocks} text, ${numToolUseBlocks} tool_use, stream=${streamEndReason ?? 'no_stream_end'}`)
    ...
    if (!hasToolUse) {
      ...
      console.log(`[agent] turn ended: no_tool_use (session=${sessionId}, stream=${streamEndReason ?? 'no_stream_end'}, details=${JSON.stringify(streamEndDetails ?? {})})`)
      const { turnCost, sessionCost } = await computeCosts(sessionId, turnCostEntries)
      onChunk?.({ type: 'done', usage: totalUsage, cost: { turn: turnCost, session: sessionCost } })
      break
    }
    ...
    for (let i = 0; i < toolUseBlocks.length; i++) {
      ...
      if (options.checkAbort?.()) {
        console.log(`[agent] turn ended: abort_between_tools (session=${sessionId}, stream=${streamEndReason ?? 'n/a'})`)
    ...
    if (options.checkAbort?.()) {
      console.log(`[agent] turn ended: abort_after_tools (session=${sessionId}, stream=${streamEndReason ?? 'n/a'})`)
      const { turnCost, sessionCost } = await computeCosts(sessionId, turnCostEntries)
      onChunk?.({ type: 'done', usage: totalUsage, cost: { turn: turnCost, session: sessionCost } })
      return { messages, usage: totalUsage, aborted: true }
    }

    // log tool_loop_continue — tools executed, going back for another LLM call
    console.log(`[agent] iteration ended: tool_loop_continue (session=${sessionId}, stream=${streamEndReason ?? 'n/a'}, tools=${toolUseBlocks.map(b => b.name).join(',')})`)
  }
