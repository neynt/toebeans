...
describe('runAgentTurn queued message handling', () => {
  ...
  test('calls onUserMessagePersisted after saving the initial user message', async () => {
    const callbackOrder: string[] = []
    setupSessionMock({
      appendMessage: async (_sid: string, msg: Message) => {
        if (msg.role === 'user' && msg.content[0]?.type === 'text' && msg.content[0].text === 'hello') {
          callbackOrder.push('append')
        }
      },
    })
    ...
    expect(callbackOrder).toEqual(['append', 'callback'])
  })

  test('default persistence keeps learn-turn prompts and replies in the session log', async () => {
    const savedMessages: Message[] = []
    let appendEntryCalls = 0
    setupSessionMock({
      loadSystemPrompt: async () => 'existing system prompt',
      appendMessage: async (_sid: string, msg: Message) => {
    ...
    const provider = makeMockProvider([
      ...
      [
        ...
        { type: 'usage', input: 50, output: 25 },
      ],
    ])

    await runAgentTurn(
      [{ type: 'text', text: 'visible learn prompt' }],
      makeOptions({
        provider,
      }),
    )
    ...
    expect(appendEntryCalls).toBe(0)
    expect(savedMessages[0]).toEqual({
      role: 'user',
      content: [{ type: 'text', text: 'visible learn prompt' }],
    })
    ...
    expect(savedMessages[2]).toEqual({
      role: 'user',
      content: [{ type: 'tool_result', tool_use_id: 'tu_1', content: 'tool result', is_error: undefined }],
    })
