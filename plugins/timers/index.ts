...
export function parseSchedule(filename: string): { type: 'absolute' | 'daily' | 'weekly' | 'hourly'; next: Date; prev?: Date; repeat: boolean } | null {
...
export default function create(): Plugin {
  ...
  let rescanInterval: Timer | null = null
  const RESCAN_INTERVAL_MS = 24 * 60 * 60 * 1000 // daily rescan
  ...
  async function queueTimerMessage(filename: string, content: string) {
    ...
    const msg: QueuedMessage = {
      ...
      route: frontmatter.session,
      outputTarget: frontmatter.output,
      triggerNotice: `Timer fired: ${filename}`,
      automatic: true,
    }
  ...
  async function scheduleTimer(filename: string) {
    ...
    // guard against setTimeout 32-bit overflow (~24.8 days): if the delay
    // exceeds the safe limit, cap it and re-evaluate when the capped timeout
    // fires. this handles absolute timers loaded from disk on server restart
    ...
    console.log(`timers: scheduled ${filename} for ${formatLocalTime(schedule.next)} (in ${formatTimeUntil(schedule.next)})${isDeferred ? ' [deferred — will re-check in 7d]' : ''}`)

    const timeout = setTimeout(() => {
      if (isDeferred) {
        // re-evaluate: time may have passed enough, or clock may have changed
        scheduleTimer(filename)
      } else {
        fireTimer(filename, schedule)
      }
    }, delay)
  ...
  async function rescanTimers() {
    try {
      const glob = new Bun.Glob('*.md')
      for await (const filename of glob.scan(timersDir)) {
        // reschedule all timers — scheduleTimer handles deferred/near-term logic
        await scheduleTimer(filename)
      }
      console.log(`timers: daily rescan complete, ${scheduledTimers.size} timer(s) active`)
    } catch (err) {
      console.error('timers: error during rescan:', err)
    }
  }
  ...
  async function loadAllTimers() {
    try {
      ...
      // start daily rescan to pick up timers that move into the 7-day window
      rescanInterval = setInterval(() => rescanTimers(), RESCAN_INTERVAL_MS)
    } catch (err) {
...
- Absolute: 2024-02-05T14:30.md (one-shot, any future date)
