...
interface Plugin {
  name: string
  description?: string
  configSchema?: { key: string; type: string; description?: string; default?: unknown; secret?: boolean }[]
  tools?: any[]
...
// Bun.main is the entry point (e.g. /path/to/toebeans/server/index.ts)
...
const { transcribe, ensureWhisperServer, ensureTtsServer, speakStreaming } = services as {
  ...
  ensureTtsServer: (opts?: TtsOptions) => Promise<boolean>
...
export default function create(_serverContext?: any) {
  ...
  async function handleWebhook(event: TelnyxWebhookEvent) {
    ...
    switch (event_type) {
      ...
      case 'call.answered': {
        ...
        ensureTtsServer().catch(e => log.err('webhook', 'tts warmup failed', e))
        break
      }
    ...
    switch (msg.event) {
      ...
      case 'start': {
        ...
        if (call.initialMessage) {
          ...
          ensureTtsServer().catch(e => log.err('call', 'tts warmup failed', e))
  ...
  const plugin: Plugin = {
    name: 'telnyx-voice',
    configSchema: [
      { key: 'apiKey', type: 'string', description: 'Telnyx API key', secret: true },
      { key: 'connectionId', type: 'string', description: 'Telnyx connection ID' },
      { key: 'fromNumber', type: 'string', description: 'outbound caller ID number' },
      { key: 'webhookPort', type: 'number', description: 'webhook HTTP port', default: 8091 },
      { key: 'mediaWsPort', type: 'number', description: 'media WebSocket port', default: 8092 },
      { key: 'publicHost', type: 'string', description: 'public hostname for webhooks' },
      { key: 'streamBidirectionalCodec', type: 'string', description: 'audio codec (L16 or PCMU)', default: 'L16' },
      { key: 'sampleRate', type: 'number', description: 'audio sample rate (8000 or 16000)', default: 8000 },
      { key: 'silenceThresholdMs', type: 'number', description: 'silence detection threshold (ms)', default: 700 },
      { key: 'silenceEnergyThreshold', type: 'number', description: 'RMS energy threshold for silence', default: 200 },
      { key: 'voiceInstruct', type: 'string', description: 'voice instruction for TTS' },
      { key: 'voiceSeed', type: 'number', description: 'TTS voice seed' },
      { key: 'voiceTemperature', type: 'number', description: 'TTS voice temperature', default: 0.3 },
      { key: 'model', type: 'string', description: 'LLM model for voice', default: 'sonnet' },
      { key: 'recordCalls', type: 'boolean', description: 'record calls to disk', default: true },
    ],
    description: [
