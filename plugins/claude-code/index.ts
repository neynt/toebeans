...
export default function create(): Plugin {
  ...
  function spawnClaudeCode(task: string, cwd: string, outputTarget?: string, route?: string) {
    ...
    if (meta.worktree && meta.originalWorkingDir) {
      const mergeMsg = await handleWorktreeMerge(
        ...
        (conflictTask, conflictCwd) => {
          spawnClaudeCode(conflictTask, conflictCwd, meta.outputTarget, meta.route)
        },
      )
      queueNotification(mergeMsg, meta.outputTarget, `Claude Code task ${status}`, meta.route)
    } else {
      queueNotification(
  ...
  return {
    name: 'claude-code',
    configSchema: [
      { key: 'notifyTarget', type: 'string', description: 'route output target for notifications (e.g. discord:channelId)' },
      { key: 'worktreeBase', type: 'string', description: 'base directory for git worktrees' },
      { key: 'model', type: 'string', description: 'Claude model to use' },
    ],
