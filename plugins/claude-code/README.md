...
when a session exits, a completion notification is injected into the parent toebeans conversation via the plugin's input generator. the notification includes `metadata.triggerEvent` (e.g. `"Claude Code completed: <task>"`) so the server can dispatch a `trigger_notice` to the output channel before the agent turn begins.
...
| `notifyTarget` | — | fallback output routing for completion notifications (per-task routing is automatic via `context.outputTarget`) |
