...
| tool | description |
|------|-------------|
| `gmail_search` | search messages using gmail query syntax. returns id, thread, labels, from, to, subject, date, snippet. drafts are auto-excluded unless the query explicitly mentions them. |
| `gmail_read` | fetch full message by id. returns labels, headers, and body text (extracts plain text from multipart MIME, falls back to stripped HTML). |
...
```json5
{
  allowSend: false,           // expose gmail_send (default: drafts only)
  fromEmail: "me",            // From: address; "me" resolves to the authenticated account
...
- the `From:` header defaults to `"me"` (Gmail's shortcut for the authenticated account). set `fromEmail` and `fromName` in plugin config to use a specific display name/address.
...
- `gmail_search` auto-appends `-in:draft` to exclude drafts unless your query explicitly uses `in:draft`, `is:draft`, or `label:draft`. use `gmail_drafts_list` for draft-specific searches.
- `gmail_search` and `gmail_read` both return label IDs (e.g. `DRAFT`, `SENT`, `INBOX`) so you can tell what kind of message you're looking at.
