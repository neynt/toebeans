...
import { encodeBase64Url, decodeBase64Url, stripHtml, extractBody, getHeader, buildMessage, queryMentionsDrafts } from './index.ts'
...
describe('buildMessage', () => {
  ...
  test('defaults From header to "me" when no identity is supplied', async () => {
    const { raw } = await buildMessage({
      to: 'bob@example.com',
      subject: 'Hello',
      body: 'hi',
  ...
  test('uses supplied From identity when provided', async () => {
    const { raw } = await buildMessage(
      { to: 'bob@example.com', subject: 'Hello', body: 'hi' },
      { email: 'jane@example.com', name: 'Jane Doe' },
    )
    expect(decodeRaw(raw)).toContain('From: Jane Doe <jane@example.com>')
  })

  test('omits display name when only email is supplied', async () => {
    const { raw } = await buildMessage(
      { to: 'bob@example.com', subject: 'Hello', body: 'hi' },
      { email: 'jane@example.com' },
    )
    expect(decodeRaw(raw)).toContain('From: jane@example.com')
    expect(decodeRaw(raw)).not.toContain('undefined')
  })
...
describe('queryMentionsDrafts', () => {
  test('detects in:draft', () => {
    expect(queryMentionsDrafts('from:alice in:draft')).toBe(true)
  })

  test('detects is:draft', () => {
    expect(queryMentionsDrafts('is:draft subject:hello')).toBe(true)
  })

  test('detects label:draft', () => {
    expect(queryMentionsDrafts('label:draft')).toBe(true)
  })

  test('detects label:drafts', () => {
    expect(queryMentionsDrafts('label:drafts')).toBe(true)
  })

  test('is case-insensitive', () => {
    expect(queryMentionsDrafts('IN:DRAFT')).toBe(true)
    expect(queryMentionsDrafts('Is:Draft')).toBe(true)
  })

  test('returns false for normal queries', () => {
    expect(queryMentionsDrafts('from:alice subject:hello')).toBe(false)
  })

  test('returns false when draft appears in non-operator context', () => {
    expect(queryMentionsDrafts('subject:draft proposal')).toBe(false)
    expect(queryMentionsDrafts('draft document')).toBe(false)
  })
})
...
describe('gmail plugin tool registration', () => {
  ...
  test('default config hides gmail_send', async () => {
    const create = (await import('./index.ts')).default
    const defaultPlugin = create()
    expect(defaultPlugin.tools!.find(t => t.name === 'gmail_send')).toBeUndefined()
  })

  beforeAll(async () => {
    const create = (await import('./index.ts')).default
    plugin = create()
    await plugin.init?.({ allowSend: true })
  })
