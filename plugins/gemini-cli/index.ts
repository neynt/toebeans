...
interface MetaFile {
  ...
  /** OutputTarget of the toebeans session that spawned this task (for routing notifications back). */
  outputTarget?: string
  /** Canonical route of the spawning conversation (e.g. "discord:example-guild-general-123"). */
  route?: string
}
...
function generateSessionId(): string {
  ...
  return `${formatLocalFilenameTimestamp(now)}_${rand}`
}
...
export default function create(): Plugin {
  ...
  function queueNotification(text: string, outputTarget?: string, triggerNotice?: string, route?: string) {
    ...
    messageQueue.push({
      ...
      outputTarget: outputTarget ?? config?.notifyTarget,
      route,
      triggerNotice,
      automatic: true,
    })
  ...
  function spawnGeminiCli(task: string, cwd: string, outputTarget?: string, route?: string) {
    ...
    if (meta.worktree && meta.originalWorkingDir) {
      const mergeMsg = await handleWorktreeMerge(
        ...
        (conflictTask, conflictCwd) => {
          spawnGeminiCli(conflictTask, conflictCwd, meta.outputTarget, meta.route)
        },
      )
      queueNotification(mergeMsg, meta.outputTarget, `Gemini CLI task ${status}`, meta.route)
    } else {
      queueNotification(
  ...
  return {
    name: 'gemini-cli',
    configSchema: [
      { key: 'notifyTarget', type: 'string', description: 'route output target for notifications' },
      { key: 'worktreeBase', type: 'string', description: 'base directory for git worktrees' },
      { key: 'model', type: 'string', description: 'Gemini model (auto, pro, flash, flash-lite)', default: 'auto' },
    ],
...
- Requires GEMINI_API_KEY or Google OAuth configured in ~/.gemini/.`,
    ...
    async status(): Promise<PluginStatus | null> {
      return getCodingAgentStatus(LOG_DIR)
    },
