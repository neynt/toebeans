...
describe('renderToolBatch', () => {
  ...
  test('shows all tools when batch has 5 or fewer', () => {
    const batch: ToolBatchEntry[] = Array.from({ length: 5 }, (_, i) => ({
      name: `tool_${i}`, summary: '', inputTokens: 1, resultTokens: 1,
    }))
    const result = renderToolBatch(batch, false)
    for (let i = 0; i < 5; i++) {
      expect(result).toContain(`tool_${i}`)
    }
    expect(result).not.toContain('hidden')
  })

  test('only shows last 5 tools when batch exceeds 5', () => {
    const batch: ToolBatchEntry[] = Array.from({ length: 8 }, (_, i) => ({
      name: `tool_${i}`, summary: '', inputTokens: 1, resultTokens: i < 6 ? 1 : undefined,
    }))
    const result = renderToolBatch(batch, false)
    // first 3 should not have detail lines
    ...
    expect(result).toContain('3 earlier tools hidden')
    // summary line names all tools
    expect(result).toContain('tool_0 → tool_1 → tool_2 → tool_3 → tool_4 → tool_5 → tool_6 → tool_7')
  })

  test('preserves overall progress counter with truncated display', () => {
    const batch: ToolBatchEntry[] = Array.from({ length: 10 }, (_, i) => ({
      name: `tool_${i}`, summary: '', inputTokens: 1, resultTokens: i < 7 ? 1 : undefined,
    }))
    const result = renderToolBatch(batch, false)
    // progress should reflect all 10 tools, not just the 5 shown
    expect(result).toContain('7/10 done')
    expect(result).toContain('5 earlier tools hidden')
    // summary line present
    expect(result).toContain('tool_0 → tool_1 → tool_2 →')
  })

  test('hidden note uses singular when only 1 tool hidden', () => {
    const batch: ToolBatchEntry[] = Array.from({ length: 6 }, (_, i) => ({
      name: `tool_${i}`, summary: '', inputTokens: 1, resultTokens: 1,
    }))
    const result = renderToolBatch(batch, true)
    expect(result).toContain('1 earlier tool hidden')
    expect(result).not.toContain('tools hidden')
  })

  test('finished batch with many tools shows correct done count', () => {
    const batch: ToolBatchEntry[] = Array.from({ length: 26 }, (_, i) => ({
      name: `tool_${i}`, summary: '', inputTokens: 1, resultTokens: 1,
    }))
    const result = renderToolBatch(batch, true)
    expect(result).toContain('done — 26 tools')
    expect(result).toContain('21 earlier tools hidden')
    // only last 5 shown
    expect(result).toContain('tool_25')
    expect(result).toContain('tool_21')
    expect(result).not.toContain('✅ tool_20')
    // summary line present
    expect(result).toContain('tool_0 → tool_1 →')
  })

  test('summary line deduplicates consecutive runs of the same tool', () => {
    const batch: ToolBatchEntry[] = [
      { name: 'bash', summary: 'cmd1', inputTokens: 1, resultTokens: 1 },
      { name: 'bash', summary: 'cmd2', inputTokens: 1, resultTokens: 1 },
      { name: 'read_file', summary: '/a', inputTokens: 1, resultTokens: 1 },
      { name: 'read_file', summary: '/b', inputTokens: 1, resultTokens: 1 },
      { name: 'bash', summary: 'cmd3', inputTokens: 1, resultTokens: 1 },
      { name: 'bash', summary: 'cmd4', inputTokens: 1, resultTokens: 1 },
      { name: 'write_file', summary: '/c', inputTokens: 1, resultTokens: 1 },
    ]
    const result = renderToolBatch(batch, true)
    // consecutive bash×2, read_file×2, bash×2 should collapse
    expect(result).toContain('bash → read_file → bash → write_file')
    expect(result).toContain('2 earlier tools hidden')
  })

  test('no summary line when batch has 5 or fewer tools', () => {
    const batch: ToolBatchEntry[] = Array.from({ length: 5 }, (_, i) => ({
      name: `tool_${i}`, summary: '', inputTokens: 1, resultTokens: 1,
    }))
    const result = renderToolBatch(batch, true)
