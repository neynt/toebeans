...
const ACTION_ALIASES: Record<string, string> = {
  ...
  // press_and_hold aliases
  hold: 'press_and_hold',
  long_press: 'press_and_hold',
  press_hold: 'press_and_hold',
  // get_bounds aliases
  bounds: 'get_bounds',
  bounding_box: 'get_bounds',
  bbox: 'get_bounds',
  // switch_frame aliases
  frame: 'switch_frame',
  iframe: 'switch_frame',
  // switch_tab aliases
...
function launchArgs(headful?: boolean): string[] {
  const args = [
    '--disable-blink-features=AutomationControlled',
    '--disable-dev-shm-usage', // avoid /dev/shm exhaustion crashes
    '--disable-gpu',           // avoid GPU process crashes on heavy pages
  ]
  if (config.remoteDebuggingPort) {
    args.push(`--remote-debugging-port=${config.remoteDebuggingPort}`)
  }
  // Chrome's native Wayland backend hangs during init when launched from
  // background processes (e.g. the toebeans server) outside the compositor's
  // logind session. Force X11/XWayland for headful sessions to get a visible
  // window reliably.
  if (headful) {
...
async function cleanStaleWindowPlacement(dataDir: string): Promise<void> {
  // Chrome 146+ crashes with "Failed global descriptor lookup: 7" (SIGTRAP) when
  // launching a persistent context that has saved window_placement in Preferences.
  // The saved placement triggers a display/compositor code path whose child process
  // expects a shared-memory FD that Patchright's pipe-based launch doesn't provide.
  // Stripping window_placement before launch lets Chrome start with default placement.
  ...
  } catch { /* no Preferences yet — first launch */ }
}

export function resolveHeadless(headful?: boolean): boolean {
  if (headful !== undefined) return !headful
  return config.headless ?? true
}

async function makePersistentContext(name: string, headful?: boolean): Promise<BrowserContext> {
  const dataDir = sessionDataDir(name)
  await mkdir(dataDir, { recursive: true })
  await cleanStaleLocks(dataDir)
  await cleanStaleWindowPlacement(dataDir)

  const ctx = await chromium.launchPersistentContext(dataDir, {
    channel: 'chrome',
    headless: resolveHeadless(headful),
    args: launchArgs(headful),
    viewport: { width: 1920, height: 1080 },
    locale: config.locale ?? 'en-US',
    timezoneId: config.timezone ?? 'America/New_York',
    userAgent: USER_AGENT,
...
interface Session {
  ...
  persistent: boolean
  sessionName?: string
  dedicatedBrowser?: Browser // headful ephemeral sessions get their own browser instance
}

/** Tracks which pages were opened as popups and their opener page. */
const popupOpeners = new WeakMap<Page, Page>()
...
async function closeSession(id: string): Promise<void> {
  ...
  try {
    ...
    if (s.dedicatedBrowser) {
      try { await s.dedicatedBrowser.close() } catch { /* already closed */ }
    }
...
export function htmlToMarkdown(html: string): string {
  ...
  let content = td.turndown(html)
    // strip markdown image refs with data URIs or blob URLs
    .replace(/!\[([^\]]*)\]\((data|blob):[^)]+\)/g, '')
    // strip inline URL-encoded SVG sludge and long data-URI strings that leak
    // through CSS backgrounds, style attrs, etc. (anything > 64 chars of data:)
    .replace(/data:[a-zA-Z/+;=0-9]+[A-Za-z0-9%+/=\-_.~,:#()]{64,}/g, '[data-uri]')
    // strip url()-wrapped encoded content that Turndown may preserve from styles
    .replace(/url\(["']?data:[^)]{64,}\)/gi, '')
    .split('\n')
    .map(l => l.trim())
    .filter(l => !/^\*(\s+\*)*\s*$/.test(l))
    .join('\n')
...
async function extractMarkdown(page: Page, selector?: string): Promise<string> {
  const html = await safeEvaluate(page, (sel: string | undefined) => {
    ...
    // strip boilerplate nav/footer regions
    root.querySelectorAll('nav, footer, [role="navigation"], [role="contentinfo"]')
      .forEach(el => el.remove())

    // strip map containers (Leaflet, Google Maps, Mapbox, generic)
    root.querySelectorAll(
      '.leaflet-container, .mapboxgl-map, .gm-style, .map-container, ' +
      '[class*="MapContainer"], [id*="map-canvas"], [class*="leaflet-"], ' +
      '[class*="mapbox"], [role="application"][aria-label*="Map"], ' +
      '[role="application"][aria-label*="map"]'
    ).forEach(el => el.remove())

    // strip images that are purely decorative or noisy (tracking pixels,
    // spacers, icons, map tiles, etc.) — keep images with meaningful alt text
    root.querySelectorAll('img').forEach(el => {
      const src = el.getAttribute('src') || ''
      const alt = (el.getAttribute('alt') || '').trim()
      const w = parseInt(el.getAttribute('width') || '0', 10)
      const h = parseInt(el.getAttribute('height') || '0', 10)
      // remove tracking pixels and spacers (1x1 or 0x0)
      if ((w <= 1 && h <= 1) || (w === 0 && h === 0 && !alt)) {
        el.remove()
        return
      }
      // remove data-URI and blob images (already stripped from markdown, but
      // removing here avoids turndown processing overhead)
      if (src.startsWith('data:') || src.startsWith('blob:')) {
        el.remove()
        return
      }
      // remove map tile images
      if (/tile|maps\.(google|googleapis)|mapbox|openstreetmap/.test(src)) {
        el.remove()
        return
      }
    })
...
/** evaluate with a 30s timeout. Does NOT kill the browser — the page remains usable. */
async function safeEvaluate(page: Page, fn: string | ((...args: any[]) => any), arg?: any): Promise<any> {
  return await Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error('page.evaluate() timed out')), EVALUATE_TIMEOUT_MS)
    ),
  ])
}
...
export default function create(): Plugin {
  return {
    name: 'browser',
    configSchema: [
      { key: 'locale', type: 'string', description: 'browser locale' },
      { key: 'timezone', type: 'string', description: 'browser timezone' },
      { key: 'sessionTimeoutMs', type: 'number', description: 'ephemeral session timeout (ms)' },
      { key: 'persistentTimeoutMs', type: 'number', description: 'persistent session timeout (ms)' },
      { key: 'persistentMaxAgeDays', type: 'number', description: 'max age for persistent sessions (days)' },
      { key: 'navigationTimeout', type: 'number', description: 'page navigation timeout (ms)' },
      { key: 'selectorTimeout', type: 'number', description: 'selector wait timeout (ms)' },
      { key: 'downloadTimeout', type: 'number', description: 'download timeout (ms)' },
      { key: 'maxContentLength', type: 'number', description: 'max content length for extraction' },
      { key: 'remoteDebuggingPort', type: 'number', description: 'CDP remote debugging port' },
      { key: 'headless', type: 'boolean', description: 'run browser headless', default: true },
    ],
    description: [
      ...
      // --- browser_spawn ---
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            headful: { type: 'boolean', description: 'Launch with a visible browser window instead of headless. Useful for sites with aggressive bot detection (e.g. StreetEasy, Apartments.com). Overrides the config-level headless setting for this session.' },
          },
        },
        async execute(input: unknown, _ctx: ToolContext): Promise<ToolResult> {
          const { url, session_name, headful } = input as { url?: string; session_name?: string; headful?: boolean }
          ...
          if (session_name && sessions.has(session_name)) {
            ...
            const tabCount = existing.context.pages().length
            if (tabCount > 1) result.tab_count = tabCount
            return { content: JSON.stringify(result, null, 2) }
          }

          const id = session_name ?? newId()

          const doSpawn = async (): Promise<ToolResult> => {
            ...
            if (persistent) {
              ...
              context = await makePersistentContext(session_name!, headful)
            } else if (headful && resolveHeadless() /* config default is headless */) {
              // headful override on an ephemeral session: launch a dedicated browser
              // so we don't pollute the shared headless singleton
              dedicatedBrowser = await chromium.launch({
                channel: 'chrome',
                headless: false,
                args: launchArgs(true),
                timeout: LAUNCH_TIMEOUT_MS,
              })
              context = await makeContext(dedicatedBrowser)
            } else {
            ...
            const session: Session = {
              ...
              dedicatedBrowser: dedicatedBrowser ?? undefined,
            }
            sessions.set(id, session)

            // track new pages/popups opened via window.open, target="_blank", etc.
            context.on('page', (newPage: Page) => {
              // record which page was active when the popup opened
              popupOpeners.set(newPage, session.page as Page)
              console.log(`[browser] new tab/popup in session ${id}: ${newPage.url()}`)
            })
            ...
            const result: Record<string, unknown> = {
              ...
              persistent,
              headful: !resolveHeadless(headful),
            }
          ...
          try {
            return await withTimeout(doSpawn(), 'browser_spawn')
          } catch (err: unknown) {
            ...
            actions: {
              ...
              items: {
                ...
                anyOf: [
                  ...
                  {
                    properties: {
                      type: { const: 'hover', description: 'Hover over an element (triggers :hover styles, tooltips, menus)' },
                  ...
                  {
                    properties: {
                      type: { const: 'mouse_move', description: 'Move mouse to a selector or viewport coordinates with optional path interpolation' },
                      selector: { type: 'string', description: 'CSS selector to move to (centers on element bounding box). Alternative to x/y.' },
                      x: { type: 'number', description: 'X coordinate (CSS pixels, relative to viewport). Required if no selector.' },
                      y: { type: 'number', description: 'Y coordinate (CSS pixels, relative to viewport). Required if no selector.' },
                      steps: { type: 'integer', description: 'Number of intermediate mousemove events (default: 1). Higher = smoother path.' },
                      jitter: { type: 'number', description: 'Max random pixel offset added to each intermediate step (default: 0)' },
                    },
                    required: ['type'],
                    additionalProperties: false,
                  },
                  {
                    properties: {
                      type: { const: 'mouse_down', description: 'Press mouse button without releasing (pair with mouse_up)' },
                  ...
                  {
                    properties: {
                      type: { const: 'mouse_up', description: 'Release mouse button' },
                      button: { type: 'string', enum: ['left', 'right', 'middle'], description: 'Mouse button (default: left)' },
                    },
                    required: ['type'],
                    additionalProperties: false,
                  ...
                  {
                    properties: {
                      type: { const: 'drag', description: 'Click-and-drag from source to destination. Provide source as selector or x/y, and destination as to_selector or to_x/to_y.' },
                      selector: { type: 'string', description: 'CSS selector for drag source element' },
                      ...
                      to_selector: { type: 'string', description: 'CSS selector for drop target element' },
                      to_x: { type: 'number', description: 'Destination X coordinate (alternative to to_selector)' },
                      to_y: { type: 'number', description: 'Destination Y coordinate (alternative to to_selector)' },
                      steps: { type: 'integer', description: 'Interpolation steps during drag movement (default: 10)' },
                      hold_ms: { type: 'number', description: 'Hold duration at source before dragging, in ms (default: 0)' },
                    },
                    required: ['type'],
                    additionalProperties: false,
                  },
                  ...
                  {
                    properties: {
                      type: { const: 'press_and_hold', description: 'Move to an element or coordinates, press and hold the mouse button for a duration, then release. Designed for "press and hold to verify" anti-bot challenges.' },
                      selector: { type: 'string', description: 'CSS selector to press and hold on (centers on element). Alternative to x/y.' },
                      ...
                      hold_ms: { type: 'number', description: 'How long to hold in ms (default: 3000). Add some randomness for realism.' },
                      steps: { type: 'integer', description: 'Mouse movement interpolation steps to reach target (default: 20)' },
                      jitter: { type: 'number', description: 'Random pixel offset on intermediate mouse move steps (default: 2)' },
                      button: { type: 'string', enum: ['left', 'right', 'middle'], description: 'Mouse button (default: left)' },
                    },
                    required: ['type'],
                    additionalProperties: false,
                  ...
                  {
                    properties: {
                      type: { const: 'get_bounds', description: 'Get the bounding box of an element. Returns {x, y, width, height, centerX, centerY} in viewport coordinates. Useful for planning coordinate-based mouse interactions.' },
                  ...
                  {
                    properties: {
                      type: { const: 'switch_frame', description: 'Switch to an iframe for subsequent actions. Anti-bot challenges (Cloudflare Turnstile, DataDome, etc.) typically render inside iframes. Use selector or url to identify the frame, or pass neither to switch back to the main frame.' },
                      selector: { type: 'string', description: 'CSS selector for the iframe element' },
                      url: { type: 'string', description: 'URL substring to match against frame URLs (alternative to selector)' },
                    },
                    required: ['type'],
                    additionalProperties: false,
                  },
                  {
                    properties: {
                      type: { const: 'list_tabs', description: 'List all open tabs/windows in this browser session. Returns index, URL, title, and whether each tab is the currently active one.' },
                    },
                    required: ['type'],
                    additionalProperties: false,
                  },
                  {
                    properties: {
                      type: { const: 'switch_tab', description: 'Switch to a different tab/window by index (from list_tabs) or URL substring. Resets frame context to the main frame of the target tab.' },
                      index: { type: 'integer', description: 'Tab index from list_tabs (0-based)' },
                      url: { type: 'string', description: 'URL substring to match (alternative to index)' },
                    },
                    required: ['type'],
                    additionalProperties: false,
                  },
                  {
                    properties: {
                      type: { const: 'new_tab', description: 'Open a new empty tab, optionally navigating to a URL. The new tab becomes the active tab.' },
                  ...
                  {
                    properties: {
                      type: { const: 'close_tab', description: 'Close the current tab. Switches to the next available tab. Cannot close the last remaining tab.' },
                    },
                    required: ['type'],
                    additionalProperties: false,
                  },
        ...
        async execute(input: unknown, _ctx: ToolContext): Promise<ToolResult> {
          ...
          try {
            ...
            const doWork = async (): Promise<ToolResult> => {
              ...
              for (const action of actions) {
                ...
                switch (action.type) {
                  ...
                  case 'click':
                    ...
                    await page.click(action.selector, { timeout: SELECTOR_TIMEOUT(), noWaitAfter: true })
                    break
                  ...
                  case 'hover':
                    ...
                    await page.hover(action.selector, { timeout: SELECTOR_TIMEOUT() })
                    break

                  case 'mouse_move': {
                    // resolve target coordinates from selector or explicit x/y
                    let targetX: number, targetY: number
                    if (action.selector) {
                      const box = await page.locator(action.selector).boundingBox({ timeout: SELECTOR_TIMEOUT() })
                      if (!box) throw new Error(`mouse_move: element not visible: ${action.selector}`)
                      ...
                      throw new Error('mouse_move requires selector or x/y coordinates')
                    }
                    const steps = action.steps ?? 1
                    if (action.jitter && steps > 1) {
                      for (let i = 1; i <= steps; i++) {
                        if (i < steps) {
                          const t = i / steps
                          const ix = targetX * t + (Math.random() - 0.5) * 2 * action.jitter
                          const iy = targetY * t + (Math.random() - 0.5) * 2 * action.jitter
                          await page.mouse.move(Math.max(0, ix), Math.max(0, iy))
                        } else {
                  ...
                  case 'mouse_down':
                    await page.mouse.down({ button: (action.button as 'left' | 'right' | 'middle') ?? 'left' })
                    break

                  case 'mouse_up':
                    await page.mouse.up({ button: (action.button as 'left' | 'right' | 'middle') ?? 'left' })
                    break

                  case 'drag': {
                    const steps = action.steps ?? 10
                    const button = (action.button as 'left' | 'right' | 'middle') ?? 'left'

                    // Resolve source position
                    let srcX: number, srcY: number
                    if (action.selector) {
                      const box = await page.locator(action.selector).boundingBox({ timeout: SELECTOR_TIMEOUT() })
                      if (!box) throw new Error(`drag: source element not visible: ${action.selector}`)
                      ...
                      throw new Error('drag requires source: selector or x/y coordinates')
                    }

                    // Resolve destination position
                    let dstX: number, dstY: number
                    if (action.to_selector) {
                      const box = await page.locator(action.to_selector).boundingBox({ timeout: SELECTOR_TIMEOUT() })
                      if (!box) throw new Error(`drag: target element not visible: ${action.to_selector}`)
                      dstX = box.x + box.width / 2
                      dstY = box.y + box.height / 2
                    } else if (action.to_x != null && action.to_y != null) {
                      dstX = action.to_x
                      dstY = action.to_y
                    } else {
                      throw new Error('drag requires destination: to_selector or to_x/to_y coordinates')
                    }

                    // Execute drag sequence
                    await page.mouse.move(srcX, srcY)
                    await page.mouse.down({ button })
                    if (action.hold_ms) await new Promise(r => setTimeout(r, action.hold_ms))
                    await page.mouse.move(dstX, dstY, { steps })
                  ...
                  case 'download': {
                    ...
                    // prevent unhandled rejection if withTimeout() or hard timeout wins the race
                    dlPromise.catch(() => {})
                    try {
                      if (action.selector) {
                  ...
                  case 'bitwarden_fill': {
                    ...
                    // Password stays in the DOM so multi-step flows work (e.g. separate
                    // submit click, JS validation, CAPTCHA gates). Secret leakage is
                    // prevented by extractMarkdown, which redacts type="password" and
                    // sensitive-name fields to "••••••" before the value ever reaches
                    // tool output or session storage.
                  ...
                  case 'press_and_hold': {
                    ...
                    if (action.selector) {
                      const box = await page.locator(action.selector).boundingBox({ timeout: SELECTOR_TIMEOUT() })
                      if (!box) throw new Error(`press_and_hold: element not visible: ${action.selector}`)
                      // slight random offset from center for human-likeness
                      phX = box.x + box.width / 2 + (Math.random() - 0.5) * box.width * 0.1
                      phY = box.y + box.height / 2 + (Math.random() - 0.5) * box.height * 0.1
                    } else if (action.x != null && action.y != null) {
                      phX = action.x
                      phY = action.y
                    } else {
                      throw new Error('press_and_hold requires selector or x/y coordinates')
                    }

                    const phButton = (action.button as 'left' | 'right' | 'middle') ?? 'left'
                    const phSteps = action.steps ?? 20
                    const phJitter = action.jitter ?? 2
                    const holdMs = action.hold_ms ?? 3000

                    // Move to target with human-like interpolation
                    if (phJitter && phSteps > 1) {
                      for (let i = 1; i <= phSteps; i++) {
                        if (i < phSteps) {
                          const t = i / phSteps
                          const ix = phX * t + (Math.random() - 0.5) * 2 * phJitter
                          const iy = phY * t + (Math.random() - 0.5) * 2 * phJitter
                          await page.mouse.move(Math.max(0, ix), Math.max(0, iy))
                        } else {
                      ...
                      await page.mouse.move(phX, phY, { steps: phSteps })
                    }

                    // Press, hold, release
                    await page.mouse.down({ button: phButton })

                    // During hold, add tiny micro-movements to look human
                    const holdEnd = Date.now() + holdMs
                    while (Date.now() < holdEnd) {
                      const remaining = holdEnd - Date.now()
                      const sleepMs = Math.min(150 + Math.random() * 200, remaining)
                      if (sleepMs <= 0) break
                      await new Promise(r => setTimeout(r, sleepMs))
                      // micro-drift: ±1-2px from hold position
                      const drift = 1 + Math.random()
                      await page.mouse.move(
                        phX + (Math.random() - 0.5) * 2 * drift,
                        phY + (Math.random() - 0.5) * 2 * drift,
                      )
                    }
                    // return to exact position before release
                    await page.mouse.move(phX, phY)
                    await page.mouse.up({ button: phButton })
                  ...
                  case 'get_bounds': {
                    if (!action.selector) throw new Error('get_bounds requires selector')
                    const box = await page.locator(action.selector).boundingBox({ timeout: SELECTOR_TIMEOUT() })
                    if (!box) throw new Error(`get_bounds: element not visible: ${action.selector}`)
                    evalResults.push(JSON.stringify({
                      action: 'get_bounds',
                      selector: action.selector,
                      x: box.x,
                  ...
                  case 'list_tabs': {
                    ...
                    const tabs = await Promise.all(allPages.map(async (p, i) => {
                      let title = ''
                      try { title = await p.title() } catch { /* closed */ }
                      const opener = popupOpeners.get(p)
                      const openerIndex = opener ? allPages.indexOf(opener) : undefined
                      return {
                        index: i,
                        url: p.url(),
                        title,
                        active: p === session.page || (session.page as unknown) === p,
                        ...(openerIndex !== undefined && openerIndex >= 0 ? { opened_by: openerIndex } : {}),
                      }
                    }))
                    evalResults.push(JSON.stringify({ action: 'list_tabs', tabs }))
                    break
                  }
                  ...
                  case 'switch_tab': {
                    ...
                    if (action.index !== undefined) {
                      target = allPages[action.index]
                      if (!target) throw new Error(`switch_tab: no tab at index ${action.index} (${allPages.length} tab(s) open)`)
                    } else if (action.url) {
                      target = allPages.find(p => p.url().includes(action.url!))
                      if (!target) throw new Error(`switch_tab: no tab matching URL "${action.url}". open tabs: ${allPages.map(p => p.url()).join(', ')}`)
                    } else {
                      throw new Error('switch_tab requires index or url')
                    }
                    session.page = target
                    page = target
                    evalResults.push(JSON.stringify({
                      action: 'switch_tab',
                      index: allPages.indexOf(target),
                      url: target.url(),
                  ...
                  case 'new_tab': {
                    ...
                    if (action.url) {
                      try {
                        await newPage.goto(action.url, { waitUntil: 'domcontentloaded', timeout: NAV_TIMEOUT() })
                      } catch (err: unknown) {
                        if (!(err as Error).message.includes('imeout')) throw err
                        console.warn(`[browser] nav timeout for ${action.url}, continuing`)
                      }
                    ...
                    evalResults.push(JSON.stringify({
                      action: 'new_tab',
                      index: context.pages().indexOf(newPage),
                      url: newPage.url(),
                      title: await newPage.title(),
                      total_tabs: context.pages().length,
                    }))
                    break
                  }
                  ...
                  case 'close_tab': {
                    const allPages = context.pages()
                    if (allPages.length <= 1) throw new Error('close_tab: cannot close the last tab')
                    const closingIndex = allPages.indexOf(page as Page)
                    await page.close()
                    ...
                    evalResults.push(JSON.stringify({
                      ...
                      now_active: { index: nextIndex, url: page.url(), title: await page.title() },
                      total_tabs: remaining.length,
                    }))
                    break
                  }
                  ...
                  case 'switch_frame': {
                    if (!action.selector && !action.url) {
                      ...
                      const mainPage = session.context.pages()[0]
                      if (mainPage) {
                        session.page = mainPage
                        page = mainPage
                      }
                      evalResults.push(JSON.stringify({ action: 'switch_frame', frame: 'main' }))
                    } else {
                      let frame = null
                      if (action.selector) {
                        const frameElement = page.frameLocator(action.selector)
                        // verify the frame exists by trying to access it
                        const innerLocator = frameElement.locator('body')
                        try {
                          await innerLocator.waitFor({ timeout: SELECTOR_TIMEOUT() })
                        } catch {
                          throw new Error(`switch_frame: no accessible frame at selector: ${action.selector}`)
                        }
                        // get the actual Frame object from page.frames()
                        const iframeHandle = await page.$(action.selector)
                        if (iframeHandle) {
                          frame = await iframeHandle.contentFrame()
                        }
                      } else if (action.url) {
                        frame = page.frames().find(f => f.url().includes(action.url!)) ?? null
                      }
                      if (!frame) {
                        const available = page.frames().map(f => f.url()).filter(u => u !== 'about:blank')
                        throw new Error(`switch_frame: frame not found. available frames: ${JSON.stringify(available)}`)
                      }
                      // Playwright Frame has the same mouse/keyboard/evaluate API as Page.
                      const framePage = frame as unknown as Page
                      session.page = framePage
                      page = framePage
                      evalResults.push(JSON.stringify({
                        action: 'switch_frame',
                        frame_url: frame.url(),
                  ...
                  default:
                    throw new Error(`unknown action type: ${action.type}. valid types: goto, click, click_text, hover, mouse_move, mouse_down, mouse_up, drag, type, press, wait, wait_for, evaluate, screenshot, scroll, select, upload_file, download, bitwarden_fill, press_and_hold, get_bounds, switch_frame, list_tabs, switch_tab, new_tab, close_tab`)
                }
              }
              ...
              if (tabCount > 1) resultObj.tab_count = tabCount
