// telnyx-voice-2: ground-up redesign with aggressive pipelining
//
...
//
// key differences from telnyx-voice v1:
...
export default function create(_serverContext?: any) {
  ...
  async function answerCall(callControlId: string) {
    ...
    const res = await telnyxApi('POST', `/calls/${callControlId}/actions/answer`, {
      ...
      client_state: Buffer.from(JSON.stringify({ plugin: 'telnyx-voice-2' })).toString('base64'),
    })
    if (!res.ok) {
  ...
  function queueMessage(text: string, callControlId: string) {
    messageQueue.push({
      ...
      outputTarget: `telnyx-voice-2:${callControlId}`,
    })
    resolveWaiter?.()
    resolveWaiter = null
  }
  ...
  const plugin: Plugin = {
    name: 'telnyx-voice-2',
    description: [
      'Real-time phone conversation via Telnyx voice API (v2 streaming pipeline).',
